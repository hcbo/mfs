...
public class SeaweedFileSystemStore {
    ...
    public boolean createDirectory(final Path path, UserGroupInformation currentUser,
        ...
        LOG.error("createDirectory path: {} permission: {} umask: {}",
            path,
            permission,
            umask);
    ...
    public FileStatus[] listEntries(final Path path) {
        LOG.error("listEntries path: {}", path);
    ...
    public FileStatus getFileStatus(final Path path) {
        ...
        LOG.error("doGetFileStatus path:{} entry:{}", path, entry);
    ...
    public boolean deleteEntries(final Path path, boolean isDirectory, boolean recursive) {
        LOG.error("deleteEntries path: {} isDirectory {} recursive: {}",
            path,
    ...
    public void rename(Path source, Path destination) {
        ...
        LOG.error("rename source: {} destination:{}", source, destination);
    ...
    public OutputStream createFile(final Path path,
        ...
        LOG.error("createFile path: {} overwrite: {} permission: {}",
            path,
            overwrite,
            permission.toString());
        ...
        if (!overwrite) {
            ...
            LOG.error("createFile merged entry path:{} existingEntry:{}", path, existingEntry);
            if (existingEntry != null) {
                entry = FilerProto.Entry.newBuilder();
                entry.mergeFrom(existingEntry);
                entry.getAttributesBuilder().setMtime(now);
                LOG.error("createFile merged entry path:{} entry:{} from:{}", path, entry, existingEntry);
    ...
    public InputStream openFileForRead(final Path path, FileSystem.Statistics statistics,
        ...
        LOG.error("openFileForRead path:{} bufferSize:{}", path, bufferSize);
    ...
    public void setOwner(Path path, String owner, String group) {
        ...
        LOG.error("setOwner path:{} owner:{} group:{}", path, owner, group);

        FilerProto.Entry entry = lookupEntry(path);
        if (entry == null) {
            LOG.error("setOwner path:{} entry:{}", path, entry);
            return;
        }
        ...
        LOG.error("setOwner path:{} entry:{}", path, entryBuilder);
    ...
    public void setPermission(Path path, FsPermission permission) {
        ...
        LOG.error("setPermission path:{} permission:{}", path, permission);

        FilerProto.Entry entry = lookupEntry(path);
        if (entry == null) {
            LOG.error("setPermission path:{} entry:{}", path, entry);
            return;
        }
        ...
        LOG.error("setPermission path:{} entry:{}", path, entryBuilder);
